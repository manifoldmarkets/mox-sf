  ...
  const handleCancelEvent = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEvents(
          events.map((e) => (e.id === selectedEvent.id ? cancelledEvent : e))
        )
  ...
  const handleReactivateEvent = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEvents(
          events.map((e) => (e.id === selectedEvent.id ? reactivatedEvent : e))
        )
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-medium text-text-primary dark:text-text-primary-dark mb-1 font-merriweather">
                      {event.name}
                    </h3>
                    <div className="flex flex-wrap gap-x-3 sm:gap-x-4 gap-y-1 text-sm text-text-muted dark:text-text-muted-dark font-sans">
                      ...
                      {event.status && (
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap ${getStatusColor(event.status)}`}
                        >
                          {event.status}
                        </span>
                      )}
