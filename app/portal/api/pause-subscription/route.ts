...
async function notifyUserPause(userEmail: string, userName: string, resumeDate: string | null, reason: string) {
  const pauseInfo = resumeDate
...
The Mox Team
    ...
    const responseData = await response.json();
    console.log('[Pause Subscription] User notification response:', response.status, responseData);
  } catch (error) {
    console.error('[Pause Subscription] Failed to send user notification email:', error);
  }
}

// Send notification to admin when they pause a subscription for another user
async function notifyAdminPause(adminEmail: string, adminName: string, userEmail: string, userName: string, resumeDate: string | null, reason: string) {
  const pauseInfo = resumeDate
    ? `Resume on: ${new Date(resumeDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`
    : 'Paused indefinitely';
...
The Mox Team
    ...
    const responseData = await response.json();
    console.log('[Pause Subscription] Admin notification response:', response.status, responseData);
  } catch (error) {
    console.error('[Pause Subscription] Failed to send admin notification email:', error);
  }
}
...
The Mox Team
    ...
    const responseData = await response.json();
    console.log('[Resume Subscription] User notification response:', response.status, responseData);
  } catch (error) {
    console.error('[Resume Subscription] Failed to send user notification email:', error);
  }
}

// Send notification to admin when they resume a subscription for another user
async function notifyAdminResume(adminEmail: string, adminName: string, userEmail: string, userName: string) {
  try {
    const resendApiKey = process.env.RESEND_API_KEY;
...
The Mox Team
    ...
    const responseData = await response.json();
    console.log('[Resume Subscription] Admin notification response:', response.status, responseData);
  } catch (error) {
    console.error('[Resume Subscription] Failed to send admin notification email:', error);
  }
}
