...
function findMatches(discordUser: ParsedDiscordUser, people: Person[]): { best: Person | null; confidence: number; alternatives: Person[] } {
...
export default function DiscordMappingTool() {
  ...
  const [step, setStep] = useState<'input' | 'review' | 'saving' | 'done'>('input');
  ...
  function handleProcessInput() {
    ...
    const newMappings: Mapping[] = parsedUsers.map(discordUser => {
      const { best, confidence, alternatives } = findMatches(discordUser, peopleWithoutDiscord);
      return {
        discordUser,
        matchedPerson: best,
        confidence,
  ...
  function updateMapping(index: number, personId: string | null) {
    setMappings(prev => {
      const updated = [...prev];
      if (personId === null) {
        updated[index] = { ...updated[index], matchedPerson: null, status: 'skipped' };
      } else {
        const person = people.find(p => p.id === personId);
        if (person) {
          updated[index] = { ...updated[index], matchedPerson: person, status: 'confirmed' };
        }
      }
      return updated;
    });
  ...
  function toggleConfirmation(index: number) {
    setMappings(prev => {
      ...
      if (current.status === 'skipped') {
        updated[index] = { ...current, status: current.matchedPerson ? 'confirmed' : 'pending' };
      } else if (current.matchedPerson) {
        updated[index] = { ...current, status: current.status === 'confirmed' ? 'pending' : 'confirmed' };
      }
      return updated;
    });
  }
  ...
  async function handleSave() {
    const confirmedMappings = mappings.filter(m => m.status === 'confirmed' && m.matchedPerson);
    ...
    for (const batch of batches) {
      try {
        ...
        setSaveProgress({ saved: totalSaved, failed: totalFailed, total: confirmedMappings.length });
      } catch (err) {
  ...
  const stats = useMemo(() => {
    ...
    return { confirmed, pending, skipped, highConfidence, total: mappings.length };
  }, [mappings]);
      ...
      {step === 'input' && (
        <div className="space-y-4">
          ...
          <div className="text-sm text-gray-600 dark:text-gray-400">
            {peopleWithoutDiscord.length} people without Discord username • {people.length - peopleWithoutDiscord.length} already mapped
          </div>
...
olduser#1234"
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-400 space-x-4">
              <span className="text-green-600 dark:text-green-400">✓ {stats.confirmed} confirmed</span>
              <span className="text-yellow-600 dark:text-yellow-400">? {stats.pending} needs review</span>
      ...
      {step === 'saving' && (
        <div className="text-center py-12">
          <div className="text-lg text-gray-700 dark:text-gray-300 mb-4">Saving mappings...</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">
            {saveProgress.saved} of {saveProgress.total} saved
            {saveProgress.failed > 0 && <span className="text-red-600 dark:text-red-400"> ({saveProgress.failed} failed)</span>}
          </div>
      ...
      {step === 'done' && (
        <div className="text-center py-12">
          <div className="text-2xl text-green-600 dark:text-green-400 mb-4">✓ Done!</div>
...
function MappingRow({
  ...
  const confidenceColor = mapping.confidence >= 0.7
    ? 'text-green-600 dark:text-green-400'
    : mapping.confidence >= 0.5
      ? 'text-yellow-600 dark:text-yellow-400'
      : 'text-red-600 dark:text-red-400';

  const statusIcon = mapping.status === 'confirmed'
    ? '✓'
    : mapping.status === 'skipped'
      ? '○'
      : '?';

  const statusColor = mapping.status === 'confirmed'
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            {mapping.matchedPerson ? (
              <span className="text-sm text-gray-900 dark:text-gray-100">
                ...
                <span className="text-gray-500 dark:text-gray-400">({mapping.matchedPerson.email})</span>
              </span>
            ) : (
              <span className="text-sm text-gray-500 dark:text-gray-400 italic">No match found</span>
            )}
          </div>
                ...
                <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 max-h-48 overflow-y-auto shadow-lg">
                  {filteredPeople.map(person => (
                    ...
                    >
                      ...
                      <div className="text-gray-500 dark:text-gray-400 text-xs">{person.email}</div>
                    </button>
                  ))}
                </div>
              )}
